...
export function KPIStrip({ items, className }: KPIStripProps) {
  return (
    <div className={cn("flex items-stretch divide-x overflow-x-auto", className)}>
      {items.map((item, i) => (
        <div key={i} className={cn("flex-1 min-w-0 px-3 py-2", i === 0 && "pl-0")}>
          <p className="section-header mb-0.5">{item.label}</p>
          <p className={cn(
            "text-base font-bold tabular leading-none tracking-tight",
