...
import { ArrowRight, Clock, AlertTriangle, CheckCircle2, FileText, Package } from "lucide-react";
...
const urgencyStyles = {
  action: "border-l-warning bg-warning/5 text-warning",
...
export function ContextualGuidance({ items, title = "What's happening now" }: Props) {
  ...
  return (
    <div className="space-y-1.5">
      <h3 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-1">{title}</h3>
      <div className="space-y-1">
        {items.slice(0, 4).map(item => (
          <Link
            ...
            to={item.link}
            className={cn(
              "flex items-center gap-3 rounded-lg border-l-[3px] px-3 py-2.5 text-[13px] transition-colors hover:bg-surface-2",
