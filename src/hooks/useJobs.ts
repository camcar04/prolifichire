  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          job_fields(*, fields(name, crop, acreage, county, state, centroid_lat, centroid_lng, bbox_north, bbox_south, bbox_east, bbox_west, boundary_geojson, field_access_instructions(*))),
          farms(name),
          job_inputs(*),
          operation_specs(*),
          job_exceptions(*),
